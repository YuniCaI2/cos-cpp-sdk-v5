...
bool BaseReq::GenerateAclRequestBody(const Owner& owner,
  ...
  rapidxml::xml_node<>* root_node = doc.allocate_node(
      rapidxml::node_element, doc.allocate_string("AccessControlPolicy"), NULL);
  doc.append_node(root_node);

  // 1. 添加ownerid节点
  rapidxml::xml_node<>* owner_node =
      doc.allocate_node(rapidxml::node_element, "Owner", NULL);
  owner_node->append_node(doc.allocate_node(
      rapidxml::node_element, "ID", doc.allocate_string(owner.m_id.c_str())));
  if (!owner.m_display_name.empty()) {
    owner_node->append_node(
        doc.allocate_node(rapidxml::node_element, "DisplayName",
                          doc.allocate_string(owner.m_display_name.c_str())));
  }
  root_node->append_node(owner_node);

  // 2. 添加AccessControlList
  rapidxml::xml_node<>* acl_node =
      doc.allocate_node(rapidxml::node_element, "AccessControlList", NULL);
  for (std::vector<Grant>::const_iterator c_itr = acl.begin();
       c_itr != acl.end(); ++c_itr) {
    const Grant& grant = *c_itr;
    rapidxml::xml_node<>* grant_node =
        doc.allocate_node(rapidxml::node_element, "Grant", NULL);
