...
bool GetBucketResp::ParseFromXmlString(const std::string& body) {
  ...
  for (; node != NULL; node = node->next_sibling()) {
    ...
    if (node_name == kGetBucketName) {
      ...
      Content cnt;
      for (; contents_node != NULL;
           contents_node = contents_node->next_sibling()) {
        ...
        if (name == kGetBucketContentsKey) {
          ...
          SDK_LOG_WARN(
              "Unknown field in content node, field_name=%s, xml_body=%s",
        ...
        if (name == kListMultipartUploadKey) {
          ...
          for (; id_node != NULL; id_node = id_node->next_sibling()) {
            ...
            if (id_node_name == kListMultipartUploadID) {
              ...
              SDK_LOG_WARN(
                  "Unknown field in KListMultipartUploadInitiator node.");
            }
...
bool GetBucketLifecycleResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Miss root node=LifecycleConfiguration, xml_body=%s",
                body.c_str());
    return false;
  }
    ...
    if ("Rule" == node_name) {
      ...
      for (; rule_node != NULL; rule_node = rule_node->next_sibling()) {
        ...
        if ("ID" == rule_node_name) {
          ...
          LifecycleFilter filter = rule.GetFilter();
          for (; filter_node != NULL;
               filter_node = filter_node->next_sibling()) {
          ...
          rule.SetIsEnable(
              ("Enabled" == std::string(rule_node->value())) ? true : false);
          ...
          for (; trans_node != NULL; trans_node = trans_node->next_sibling()) {
            ...
            if ("Days" == trans_node_name) {
              transition.SetDays(
                  StringUtil::StringToUint64(trans_node->value()));
          ...
          for (; expir_node != NULL; expir_node = expir_node->next_sibling()) {
            ...
            if ("Days" == expir_node_name) {
              expiration.SetDays(
                  StringUtil::StringToUint64(expir_node->value()));
          ...
          for (; trans_node != NULL; trans_node = trans_node->next_sibling()) {
            ...
            if ("NoncurrentDays" == trans_node_name) {
              ...
              SDK_LOG_WARN(
                  "Unknown field in NoncurrentVersionTransition node,"
                  "field_name=%s",
                  trans_node_name.c_str());
            }
          }
          ...
          for (; expir_node != NULL; expir_node = expir_node->next_sibling()) {
            ...
            if ("NoncurrentDays" == expir_node_name) {
              ...
              SDK_LOG_WARN(
                  "Unknown field in NoncurrentVersionExpiration node,"
                  "field_name=%s",
                  expir_node_name.c_str());
            }
          }
          ...
          for (; abort_node != NULL; abort_node = abort_node->next_sibling()) {
            ...
            if ("DaysAfterInitiation" == abort_node_name) {
              ...
              SDK_LOG_WARN(
                  "Unknown field in AbortIncompleteMultipartUpload node,"
                  "field_name=%s",
                  abort_node_name.c_str());
            }
          }
      ...
      SDK_LOG_WARN(
          "Unknown field in LifecycleConfiguration node, field_name=%s",
          node_name.c_str());
    }
  }
...
bool GetBucketACLResp::ParseFromXmlString(const std::string& body) {
  return ParseFromACLXMLString(body, &m_owner_id, &m_owner_display_name,
                               &m_acl);
}
...
bool GetBucketVersioningResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Miss root node=VersioningConfiguration, xml_body=%s",
                body.c_str());
    return false;
  }
    ...
    if ("Name" == node_name) {
      ...
      for (; result_node != NULL; result_node = result_node->next_sibling()) {
        ...
        if ("Key" == result_node_name) {
          ...
          SDK_LOG_WARN(
              "Unknown field in DeleteMarker/Version node, field_name=%s.",
...
bool GetBucketLoggingResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Miss root node=BucketLoggingConfiguration, xml_body=%s",
                body.c_str());
    return false;
  }

  rapidxml::xml_node<>* loggingenabled_node =
      root->first_node("LoggingEnabled");
  if (NULL == loggingenabled_node) {
    SDK_LOG_ERR("Miss node=LoggingEnableConfiguration, xml_body=%s",
                body.c_str());
    return false;
  }
...
bool GetBucketDomainResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Miss root node=DomainConfiguration, xml_body=%s",
                body.c_str());
    return false;
  }
...
bool GetBucketWebsiteResp::ParseFromXmlRoutingRule(
    rapidxml::xml_node<>* node, RoutingRule& tmp_routingrule) {
  rapidxml::xml_node<>* rule_node = node->first_node();
  for (; rule_node != NULL; rule_node = rule_node->next_sibling()) {
    const std::string& rule_node_name = rule_node->name();
...
bool GetBucketWebsiteResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Miss root node=WebsiteConfiguration, xml_body=%s",
                body.c_str());
    return false;
  }
  ...
  for (; node != NULL; node = node->next_sibling()) {
    ...
    if (node_name == "IndexDocument") {
      ...
      for (; routingrules_node != NULL;
           routingrules_node = routingrules_node->next_sibling()) {
...
bool GetBucketInventoryResp::ParseFromXmlCOSBucketDestination(
    rapidxml::xml_node<>* node, Inventory& temp_inventory) {
  rapidxml::xml_node<>* destination_node = node->first_node();
  COSBucketDestination temp_destination;
  for (; destination_node != NULL;
       destination_node = destination_node->next_sibling()) {
...
bool GetBucketInventoryResp::ParseFromXmlOptionalFields(
    rapidxml::xml_node<>* node, Inventory& temp_inventory) {
...
bool GetBucketInventoryResp::ParseFromXmlInventoryConfiguration(
    rapidxml::xml_node<>* root, Inventory& temp_inventory) {
  rapidxml::xml_node<>* node = root->first_node();
  for (; node != NULL; node = node->next_sibling()) {
    const std::string& node_name = node->name();
    ...
    if (node_name == "Id") {
      ...
      if (StringUtil::StringToLower(IsEnabled) ==
          StringUtil::StringToLower("True"))
      ...
      if (std::string(COSBucketDestination_node->name()) ==
          "COSBucketDestination") {
        ParseFromXmlCOSBucketDestination(COSBucketDestination_node,
                                         temp_inventory);
      }
...
bool GetBucketInventoryResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Miss root node = InventoryConfiguration, xml_body=%s",
                body.c_str());
    return false;
  } else {
...
bool ListBucketInventoryConfigurationsResp::ParseFromXmlString(
  ...
  rapidxml::xml_node<>* root =
      doc.first_node("ListInventoryConfigurationResult");
  if (NULL == root) {
    SDK_LOG_ERR(
        "Miss root node = ListInventoryConfigurationResult, xml_body=%s",
        body.c_str());
    return false;
  }
  rapidxml::xml_node<>* InventoryConfiguration_node = root->first_node();
  for (; InventoryConfiguration_node != NULL;
       InventoryConfiguration_node =
           InventoryConfiguration_node->next_sibling()) {
    const std::string& InventoryConfiguration_node_name =
        InventoryConfiguration_node->name();
    if (InventoryConfiguration_node_name == "InventoryConfiguration") {
      Inventory temp_inventory;
      ParseFromXmlInventoryConfiguration(InventoryConfiguration_node,
                                         temp_inventory);
      ...
      if (StringUtil::StringToLower(IsTruncated) ==
          StringUtil::StringToLower("true"))
        SetIsTruncated(true);
      else
      ...
      SetNextContinuationToken(
          std::string(InventoryConfiguration_node->value()));
    } else {
      continue;
    }
  }
...
bool ListLiveChannelResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Missing root node ListLiveChannelResult, xml_body=%s",
                body.c_str());
    return false;
  }
  ...
  for (; node != NULL; node = node->next_sibling()) {
    ...
    if ("MaxKeys" == node_name) {
      ...
      for (; live_channel_node != NULL;
           live_channel_node = live_channel_node->next_sibling()) {
        ...
        if ("Name" == node_name) {
          ...
          SDK_LOG_WARN("Unknown field in LiveChannel, field_name=%s",
                       node_name.c_str());
          return false;
        }
      }
      ...
      SDK_LOG_WARN("Unknown field in ListLiveChannelResult, field_name=%s",
                   node_name.c_str());
      return false;
    }
  }
...
bool GetBucketIntelligentTieringResp::ParseFromXmlString(
    const std::string& body) {
  std::string tmp_body = body;
  rapidxml::xml_document<> doc;
  ...
  rapidxml::xml_node<>* root =
      doc.first_node("IntelligentTieringConfiguration");
  if (NULL == root) {
    SDK_LOG_ERR(
        "Missing root node IntelligentTieringConfiguration, xml_body=%s",
        body.c_str());
    return false;
  }

  rapidxml::xml_node<>* intel_tiering_node = root->first_node();
  for (; intel_tiering_node != NULL;
       intel_tiering_node = intel_tiering_node->next_sibling()) {
    ...
    if ("Status" == node_name) {
      ...
      for (; trans_node != NULL; trans_node = trans_node->next_sibling()) {
        ...
        if ("Days" == node_name) {
          ...
          SDK_LOG_WARN("Unknown field in Transition, field_name=%s",
                       node_name.c_str());
          return false;
        }
      }
      ...
      SDK_LOG_WARN(
          "Unknown field in IntelligentTieringConfiguration, field_name=%s",
          node_name.c_str());
      return false;
    }
  }
