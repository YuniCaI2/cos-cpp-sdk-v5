...
bool DeleteObjectsResp::ParseFromXmlString(const std::string& body) {
  ...
  for (; node != NULL; node = node->next_sibling()) {
    ...
    if ("Deleted" == node_name) {
      ...
      for (; deleted_node != NULL;
           deleted_node = deleted_node->next_sibling()) {
        ...
        if ("Key" == deleted_node_name) {
          ...
          info.m_delete_marker =
              std::string(deleted_node->value()) == "true" ? true : false;
...
// prelude = total_byte_len(4 byte) + header_byte_len(4 byte) +
// CRC32(total_byte_len + header_byte_len)(4 byte) message_data = header +
// payload
bool SelectObjectContentResp::ParseFromXmlString(const std::string& body) {
  ...
  while (body_cursor < body_length) {
    ...
    total_byte_length =
        StringUtil::GetUint32FromStrWithBigEndian(message_start);
    ...
    header_byte_length = StringUtil::GetUint32FromStrWithBigEndian(
    ...
    while (header_cursor < header_byte_length) {
      ...
      if (header_value_type != 7) {
        SDK_LOG_ERR("Invalid header value type:%u, expect 7",
                    header_value_type);
        return false;
      }
      header_cursor++;
      uint16_t header_value_len = StringUtil::GetUint16FromStrWithBigEndian(
          header_start + header_cursor);
    ...
    if (header_map.count(":message-type") > 0) {
      if (header_map[":message-type"] == "event") {
        ...
        if (event_type == "End") {
          ...
          if (event_type == "Records" || event_type == "Progress" ||
              event_type == "Stats") {
            // new message block
            SelectMessage msg;
            msg.m_event_type = std::move(event_type);
            msg.m_content_type = std::move(content_type);
            msg.payload = std::move(
                std::string(message_start + message_cursor, payload_length));
            SDK_LOG_DBG("Get event:%s, content_type:%s, payload:%s",
                        msg.m_event_type.c_str(), msg.m_content_type.c_str(),
                        msg.payload.c_str());
        ...
        SDK_LOG_ERR("Get error-message:%s, error-code:%s",
                    error_message.c_str(), error_code.c_str());
        return false;
      } else {
        SDK_LOG_ERR("Unknown message type:%s",
                    header_map[":message-type"].c_str());
        return false;
      }
    } else {
...
int SelectObjectContentResp::WriteResultToLocalFile(const std::string& file) {
  ...
  if (!ofs.is_open()) {
    SDK_LOG_ERR("Failed to open file:%s, error info:%s", file.c_str(),
                strerror(errno));
    return -1;
  }
  for (size_t i = 0; i < resp_data.size(); ++i) {
...
bool PutLiveChannelResp::ParseFromXmlString(const std::string& body) {
  ...
  // StringUtil::StringToXml(const_cast<char *>(tmp_body.c_str()), &doc) not
  // working
  if (!StringUtil::StringToXml(&tmp_body[0], &doc)) {
    SDK_LOG_ERR("Parse string to xml doc error, xml_body=%s", body.c_str());
    return false;
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Miss root node=CreateLiveChannelResult, xml_body=%s",
                body.c_str());
    return false;
  }
...
bool GetLiveChannelResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Missing root node LiveChannelConfiguration, xml_body=%s",
                body.c_str());
    return false;
  }
    ...
    if ("Description" == node_name) {
      ...
      for (; target_node != NULL; target_node = target_node->next_sibling()) {
        ...
        if ("Type" == node_name) {
          ...
          m_chan_conf.SetFragDuration(
              StringUtil::StringToInt(target_node->value()));
        } else if ("FragCount" == node_name) {
          m_chan_conf.SetFragCount(
              StringUtil::StringToInt(target_node->value()));
          ...
          SDK_LOG_WARN("Unknown field in Target, field_name=%s",
                       node_name.c_str());
          return false;
        }
      }
    } else {
      SDK_LOG_WARN("Unknown field in LiveChannelConfiguration, field_name=%s",
                   node_name.c_str());
      return false;
    }
  }
...
bool GetLiveChannelHistoryResp::ParseFromXmlString(const std::string& body) {
  ...
  if (NULL == root) {
    SDK_LOG_ERR("Missing root node LiveChannelHistory, xml_body=%s",
                body.c_str());
    delete[] cstr;
    return false;
  }
  ...
  for (; node != NULL; node = node->next_sibling()) {
    ...
    if ("LiveRecord" != node_name) {
      SDK_LOG_WARN("Unknown field in LiveChannelHistory, field_name=%s",
                   node_name.c_str());
      delete[] cstr;
      return false;
    }
    ...
    for (; record_node != NULL; record_node = record_node->next_sibling()) {
      ...
      if ("StartTime" == node_name) {
        ...
        SDK_LOG_WARN("Unknown field in LiveRecord, field_name=%s",
                     node_name.c_str());
        delete[] cstr;
        return false;
      }
  ...
  for (; node != NULL; node = node->next_sibling()) {
    ...
    if ("Status" == node_name) {
      ...
      if (status != "Idle" && status != "Live") {
        SDK_LOG_WARN("Unknown Status in LiveChannelStatus, Status:%s",
                     status.c_str());
        return false;
      }
      ...
      for (; video_node != NULL; video_node = video_node->next_sibling()) {
        ...
        if ("Width" == node_name) {
          ...
          SDK_LOG_WARN("Unknown field in Video, field_name=%s",
                       node_name.c_str());
          return false;
        }
      }
      ...
      for (; audio_node != NULL; audio_node = audio_node->next_sibling()) {
        ...
        if ("Bandwidth" == node_name) {
          ...
          SDK_LOG_WARN("Unknown field in Audio, field_name=%s",
                       node_name.c_str());
          return false;
        }
      }
    } else {
      SDK_LOG_WARN("Unknown field in LiveChannelStatus, field_name=%s",
                   node_name.c_str());
      return false;
    }
  }
